...
for line in _input:
  l = line.split(' ')
  h_cache = {}
  for card in l[0].replace('J', ''):
    h_cache[card] = h_cache.get(card, 0) + 1
  
  # Replace J with best combo card
  if 'J' in l[0]:
    b_combo_cache = [v for k, v in h_cache.items() if k != 'J']
    b_combo = max(b_combo_cache if len(b_combo_cache) else [0])
    b_card = [k for k, v in h_cache.items() if v == b_combo and k != 'J']
    b_hand = l[0].replace('J', b_card[0] if len(b_card) else 'J')
    h_cache.clear()
    for card in b_hand:
      h_cache[card] = h_cache.get(card, 0) + 1
...
def compare_hands(h1, h2):
  cv = lambda c: 'J23456789TQKA'.index(c)
  for c in range(5):
    v1 = cv(h1['hand'][c])
    v2 = cv(h2['hand'][c])
    if v1 != v2:
